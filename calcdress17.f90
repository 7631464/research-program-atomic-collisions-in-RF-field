...
read(*,*)
read(*,*) Einput,    Einput2, numepoints
read(*,*)
...
order=12									!THE ORDER OF THE B-SPLINES
...
estep=(Einput2-Einput)/dfloat(numepoints-1)
bx=bx*Gauss
wL=wL*MHz
write(*,*) 'estep=',estep/muK, Einput/muK,Einput2/muK,numepoints
write(*,*) Einput,energy
write(*,*) c6,c8,c10
...
do ienergy=1,numepoints
energy=(Einput+dfloat(ienergy-1)*estep)/ebeta
write(*,*) 'energy=',energy*ebeta
...
!write(*,*) "a=",ascurrent
write(*,*) 'asarray', energy
do i=1,ithresh
asarray(i)=-tan(zlog(sphys(i,i))/(0.d0,2.d0))/sqrt(2d0*mu*(energy*ebeta+thresholds(ithresh)-thresholds(i)))
...
do j=1,numopen
if (i==j) then
crosssection(i,j)=pi/(2d0*mu*(energy*ebeta+thresholds(ithresh)-thresholds(i)))*abs(sphys(i,j)-1d0)**2
else 
crosssection(i,j)=pi/(2d0*mu*(energy*ebeta+thresholds(ithresh)-thresholds(i)))*abs(sphys(i,j))**2
end if
end do 
end do
...
write(42,'(400e25.15)') bx/gauss,wL/(MHz),Bfield/gauss,energy*ebeta/(muK),real(asarray(ithresh)),aimag(asarray(ithresh)),dfloat(ithresh),evectorssav(istart,ithresh),dfloat(isph)!,evectorssav(101,ithresh), dfloat(isph)!,real(asarray(ithresh)-aspre)/wLstep
...
pot0=(l*(l+1.d0))/rad**2.d0+vxssgp_unret(rad*beta,-0.5430d-4*0.93d0,4700.d0)/ebeta
...
real*8              Thresholds(200), VQcal(0:1,200,200)
